...
def add_source_validation(dataset_generator):

    """
    use the same get dataset function as before, but also produce a test set in the source domain
    """

    def func(*args, **kwargs):
        tsd, ttd, vd, td, num_classes, class_names = dataset_generator(*args, **kwargs)
        train_size = int(0.8 * len(tsd))
        test_size = len(tsd) - train_size
        tsd, tstd = torch.utils.data.random_split(tsd, [train_size, test_size])
        tstd.transform = args[-1] 
        return tsd, tstd, ttd, vd, td, num_classes, class_names
...
def validate(val_loader, model, args, device) -> float:
    ...
    if (confmat):
        ...
        return top1.avg, acc_global, acc, iu

    return top1.avg, None, None, None
