...
def main(args: argparse.Namespace):
    ...
    get_dataset_with_sv = utils.add_source_validation(utils.get_dataset)

    train_source_dataset, test_source_dataset, train_target_dataset, val_dataset, test_dataset, num_classes, args.class_names = \
        get_dataset_with_sv(args.data, args.root, args.source, args.target, train_transform, val_transform)
    ...
    if args.phase == 'test':
        acc1, acc_global, acc, iu = utils.validate(test_loader, target_classifier, args, device)
        print(acc1)
        return
    ...
    for epoch in range(args.epochs):
        ...
        acc1, acc_global, acc, iu = utils.validate(val_loader, target_classifier, args, device)

        acc1_s, acc_global_s, acc_s, iu_s = utils.validate(test_source_loader, source_classifier, args, device)

        #add logs for accuracy over each class 
    ...
    print("test_acc1 = {:3.1f}".format(acc1_s))
